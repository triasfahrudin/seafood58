...
var
  FrmPerubahanStok: TFrmPerubahanStok;
...
procedure TFrmPerubahanStok.LoadAllData;
begin
  dm.ExecSQL(
  ...
  NxFlipPanel2.Caption := 'Entry Perubahan Stok untuk tanggal: ' + cal.Text;
  NxFlipPanel1.Caption := 'List Perubahan Stok untuk tanggal: ' + cal.Text;
end;

procedure TFrmPerubahanStok.FormCreate(Sender: TObject);
begin
  LoadAllData;
...
procedure TFrmPerubahanStok.btnPrevClick(Sender: TObject);
begin
  cal.AddDays(-1);
end;

procedure TFrmPerubahanStok.btnNextClick(Sender: TObject);
begin
  cal.AddDays(1);
end;

procedure TFrmPerubahanStok.gridAfterEdit(Sender: TObject; ACol,
...
procedure TFrmPerubahanStok.btnKeluarClick(Sender: TObject);
begin
  Close;
end;

procedure TFrmPerubahanStok.Delete1Click(Sender: TObject);
begin
  if bEdited then
    Exit;
  ...
  begin
    if isAllowPriv(PERUBAHAN_STOK_DELETE) = 'N' then
      ShowAuthorization(PERUBAHAN_STOK_DELETE);
    if isAllowPriv(PERUBAHAN_STOK_DELETE) = 'N' then
      Exit;
  end;
  {****************************************************************************}
  ...
  begin
    DeletePriv(PERUBAHAN_STOK_DELETE);
    Exit;
  end;
  dm.ExecSQL('delete from data_perubahan_stok where id = %d',
...
procedure TFrmPerubahanStok.gridBeforeEdit(Sender: TObject; ACol,
...
procedure TFrmPerubahanStok.calChange(Sender: TObject);
begin
  LoadAllData;
end;

procedure TFrmPerubahanStok.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure TFrmPerubahanStok.FormShow(Sender: TObject);
begin
  cmb1.Items.Clear;
  dm.ExecSQL(
    'select nama'#13 +
...
procedure TFrmPerubahanStok.cmb2Enter(Sender: TObject);
var
  currIdSatuan: integer;
...
procedure TFrmPerubahanStok.e2KeyPress(Sender: TObject; var Key: Char);
begin
  if key = #13 then
    mm1.SetFocus;
  if (key <> #8) then
...
procedure TFrmPerubahanStok.e1KeyPress(Sender: TObject; var Key: Char);
begin
  if key = #13 then
    cmb2.SetFocus;
...
procedure TFrmPerubahanStok.cmb1KeyPress(Sender: TObject; var Key: Char);
begin
  if key = #13 then
    e1.SetFocus;
end;

procedure TFrmPerubahanStok.cmb2KeyPress(Sender: TObject; var Key: Char);
begin
  if key = #13 then
    e2.SetFocus;
end;

procedure TFrmPerubahanStok.btnBaruClick(Sender: TObject);
begin
  cmb1.ItemIndex := -1;
  e1.Clear;
  cmb2.ItemIndex := 0;
...
procedure TFrmPerubahanStok.btnSimpanClick(Sender: TObject);
var
  iIdSatuan,
    iIdSubJenisMenu: integer;
begin
  ...
  begin
    MessageDlg(
      'Nama satuan ini tidak ditemukan,'#13 +
...
procedure TFrmPerubahanStok.SetControlStatus(bBaru, bSimpan: boolean);
begin
...
procedure TFrmPerubahanStok.btnBatalClick(Sender: TObject);
begin
  SetControlStatus(True, False);
  btnBaru.SetFocus;
end;

procedure TFrmPerubahanStok.cmb1Change(Sender: TObject);
var
  iIdSubJenisMenu: integer;
begin
  cmb2.ItemIndex := -1;
